...
>
> JSP引擎在调用JSP对应的_jspService时，会传递或创建9个与web开发相关的对象供_jspService使用。JSP技术的设计者为便于开发人员在编写JSP页面时获得这些web对象的引用，特意定义了9个相应的变量，开发人员在JSP页面中通过这些变量就可以快速获得这9大对象的引用。内置对象的特点是
>
> 1. 由JSP规范提供,不用编写者实例化。
> 2. 通过Web容器实现和管理
> 3. 所有JSP页面均可使用
